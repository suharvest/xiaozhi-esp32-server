...
class TTSProviderBase(ABC):
    def __init__(self, config, delete_audio_file):
        ...
        # 首句最大字符数：攒够这么多字仍没等到标点，就强制合成，不再等待。
        #
        # 原值 5 太小，会从词中间劈开：模型流式吐出「您好像输入」时正好 5 个字、
        # 「有误」后面的逗号还没到，于是播成「您好像输入」「有误，是想说…」两句。
        # 实测日志里就是这个形状。
        #
        # 它是为降低首音频延迟设的。但现在两条延迟来源都已经解决 —— EdgeLLM 接上
        # system prompt KV cache 后首轮推理 1.81s -> 0.42s，工具调用期间又有填充语
        # 顶着 —— 再用这么激进的阈值只剩下切碎句子的副作用。
        #
        # 16 的依据：中文一个短句到逗号通常 8~15 字，取 16 能让绝大多数句子等到
        # 自然标点；真遇到超长无标点的首句，仍会在 16 字处切，不会无限等待。
        self.first_sentence_max_chars = int(
            config.get("first_sentence_max_chars", 16)
        )
    ...
    def _get_segment_text(self):
        ...
        # 首句用 find() 找第一个标点加速输出；后续用 rfind() 缓冲更多文本
        punct_pos = -1
        if self.is_first_sentence:
            ...
            for punct in punctuations_to_use:
                ...
                if (pos != -1 and punct_pos == -1) or (pos != -1 and pos < punct_pos):
                    punct_pos = pos

        if punct_pos != -1:
            segment_text_raw = current_text[: punct_pos + 1]
            ...
            return segment_text
        # 首句字符数达到上限，强制切分以降低延迟
        elif self.is_first_sentence and len(current_text) >= self.first_sentence_max_chars:
